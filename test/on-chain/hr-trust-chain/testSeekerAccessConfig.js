...
// 注意：這個只適用於後端沒有真的 recover MetaMask 簽章的情況。
// 如果後端會驗證 EVM signature，需要改成用每個 address 的 EVM private key 簽 message。
...
// k6 沒有 ethers，所以這裡先用 sha256 當假 hash。
// 如果後端會嚴格比對 keccak256(accessCanonical)，這裡要改成真正 keccak256。
function fakeAccessHash(accessConfigObj) {
  const canonical = stableStringify(accessConfigObj);
  const digest = sha256(canonical, 'hex');
  return `0x${digest}`;
}
...
export default function () {
  ...
  const accessHash = fakeAccessHash(accessConfig);
  const message = `SetAccessConfig(start) for ${address} accessHash=${accessHash} ts=${ts}`;
  const signature = fakeStartSignature(message);

  // ---------- start ----------
  const startRes = http.post(
    `${API_BASE}/seeker/setAccessConfig/start`,
    JSON.stringify({
      address,
      accessConfig,
      ts,
  ...
  // ---------- finish 1 ----------
  const finish1Res = http.post(
    `${API_BASE}/seeker/setAccessConfig/finish`,
    JSON.stringify({
      token,
      address,
...
export function handleSummary(data) {
  ...
  return {
    ...
    'resultSeekerAccessConfig_run.json': JSON.stringify(row, null, 2),
    'resultSeekerAccessConfig_run.csv': `${csvHeader}\n${csvRow}\n`,
  };
}