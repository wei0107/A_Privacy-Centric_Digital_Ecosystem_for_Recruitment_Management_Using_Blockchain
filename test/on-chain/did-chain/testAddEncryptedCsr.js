...
const payloads = JSON.parse(open('./addEncryptedCsrPayloads.json'));

export const options = {
  scenarios: {
    add_encrypted_csr_test: {
      ...
      preAllocatedVUs: 100,
      maxVUs: 1000,
      exec: 'addEncryptedCSR',
      tags: {
        mode: 'add_encrypted_csr',
        rate: String(RATE),
      },
    },
  },
...
export function addEncryptedCSR() {
  const payload = payloads[__ITER % payloads.length];

  const res = http.post(
    `${BASE_URL}/test/did/add-encrypted-csr`,
...
export function handleSummary(data) {
  ...
  return {
    stdout: `add_encrypted_csr | rate=${row.rate} | throughput=${row.throughput_rps} rps | avg=${row.avg_ms} ms | p95=${row.p95_ms} ms | fail=${row.failed_rate_percent}%\n`,
    'resultAddEncryptedCSR_run.json': JSON.stringify(row, null, 2),
    'resultAddEncryptedCSR_run.csv': `${csvHeader}\n${csvRow}\n`,
  };
}