...
const payloads = JSON.parse(open('./getEncryptedCsrPayloads.json'));

export const options = {
  scenarios: {
    get_encrypted_csr_test: {
      ...
      preAllocatedVUs: 100,
      maxVUs: 1000,
      exec: 'getEncryptedCSR',
      tags: {
        mode: 'get_encrypted_csr',
        rate: String(RATE),
      },
    },
  },
...
export function getEncryptedCSR() {
  const payload = payloads[__ITER % payloads.length];

  const res = http.post(
    `${BASE_URL}/test/did/get-encrypted-csr`,
...
export function handleSummary(data) {
  ...
  return {
    stdout: `get_encrypted_csr | rate=${row.rate} | throughput=${row.throughput_rps} rps | avg=${row.avg_ms} ms | p95=${row.p95_ms} ms | fail=${row.failed_rate_percent}%\n`,
    'resultGetEncryptedCSR_run.json': JSON.stringify(row, null, 2),
    'resultGetEncryptedCSR_run.csv': `${csvHeader}\n${csvRow}\n`,
  };
}