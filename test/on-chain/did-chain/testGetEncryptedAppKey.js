...
const payloads = JSON.parse(open('./getEncryptedAppKeyPayloads.json'));

export const options = {
  scenarios: {
    get_encrypted_appkey_test: {
      ...
      preAllocatedVUs: 100,
      maxVUs: 1000,
      exec: 'getEncryptedAppKey',
      tags: {
        mode: 'get_encrypted_appkey',
        rate: String(RATE),
      },
    },
  },
...
export function getEncryptedAppKey() {
  const payload = payloads[__ITER % payloads.length];

  const res = http.post(
    `${BASE_URL}/test/did/get-encrypted-app-key`,
    JSON.stringify(payload),
    {
      ...
      tags: {
        endpoint: 'get-encrypted-app-key',
      },
    }
  );
...
export function handleSummary(data) {
  ...
  return {
    stdout: `get_encrypted_appkey | rate=${row.rate} | throughput=${row.throughput_rps} rps | avg=${row.avg_ms} ms | p95=${row.p95_ms} ms | fail=${row.failed_rate_percent}%\n`,
    'resultGetEncryptedAppKey_run.json': JSON.stringify(row, null, 2),
    'resultGetEncryptedAppKey_run.csv': `${csvHeader}\n${csvRow}\n`,
  };
}