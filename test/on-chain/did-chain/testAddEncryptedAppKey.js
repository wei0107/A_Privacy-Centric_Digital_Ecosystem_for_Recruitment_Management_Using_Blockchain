...
const payloads = JSON.parse(open('./addEncryptedAppKeyPayloads.json'));

export const options = {
  scenarios: {
    add_encrypted_appkey_test: {
      ...
      preAllocatedVUs: 100,
      maxVUs: 1000,
      exec: 'addEncryptedAppKey',
      tags: {
        mode: 'add_encrypted_appkey',
        rate: String(RATE),
      },
    },
  },
...
export function addEncryptedAppKey() {
  const payload = payloads[__ITER % payloads.length];

  const res = http.post(
    `${BASE_URL}/test/did/add-encrypted-app-key`, // 如果你有新 API 再改
...
export function handleSummary(data) {
  ...
  return {
    stdout: `add_encrypted_appkey | rate=${row.rate} | throughput=${row.throughput_rps} rps | avg=${row.avg_ms} ms | p95=${row.p95_ms} ms | fail=${row.failed_rate_percent}%\n`,
    'resultAddEncryptedAppKey_run.json': JSON.stringify(row, null, 2),
    'resultAddEncryptedAppKey_run.csv': `${csvHeader}\n${csvRow}\n`,
  };
}